(()=>{
  ...
  ready(()=>{
    ...
    function render(){
      ...
      const key=`${filter}|`+all.map(x=>`${x.index}:${x.kind}:${x.name||''}:${x.visible?1:0}:${x.selected?1:0}`).join('|');
      ...
      for(const item of items){
        ...
        const label=item.name||`${item.kind==='outline'?'Outline':'Tube'} Balloon ${item.index+1}`;
        selectBtn.innerHTML=`<span class="outliner-kind">${item.kind==='outline'?'◯':'⌁'}</span><span>${label.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}</span>`;
        selectBtn.onclick=()=>{a.selectIndex(item.index);lastKey='';render()};
        const rename=document.createElement('button');rename.className='outliner-icon';rename.title='Rename';rename.textContent='✎';rename.onclick=e=>{e.stopPropagation();if(typeof a.renameIndex!=='function'){status.textContent='Rename unavailable in this build';return}const next=prompt('Rename balloon',label);if(next&&a.renameIndex(item.index,next)){lastKey='';render()}};
        ...
        row.append(selectBtn,rename,eye,dup,del);list.appendChild(row);
      }
    }
    ...
    const style=document.createElement('style');style.textContent=`
      ...
      .outliner-row{display:grid;grid-template-columns:minmax(0,1fr) 27px 27px 27px 27px;gap:3px;align-items:center}
      ...
      .outliner-icon{padding:2px!important;font-size:13px!important;line-height:1!important}
      .outliner-delete{font-size:16px!important}
    `;document.head.appendChild(style);
    setInterval(render,220);render();
    if(status)status.textContent=`v${build} • Outliner visibility, filtering and naming`;
  });
})();
