...
const controls=new OrbitControls(camera,renderer.domElement);controls.target.set(0,0,0);controls.enableDamping=true;controls.enabled=false;controls.enableRotate=true;controls.enablePan=true;controls.enableZoom=true;controls.touches.ONE=THREE.TOUCH.ROTATE;controls.touches.TWO=THREE.TOUCH.DOLLY_PAN;
...
function uiSettings(){return{width:+$('#width').value,pressure:+$('#pressure').value,smooth:+$('#smooth').value,sides:+$('#sides').value,taper:$('#taper').checked}}
...
function resample(samples,settings){
  ...
  for(let i=0;i<path.length;i++){
    ...
    const pressureFactor=THREE.MathUtils.lerp(1,0.35+0.65*pressure,settings.pressure);
    let radius=base*pressureFactor;
    if(settings.taper){const edge=Math.min(u,1-u),t=THREE.MathUtils.smoothstep(edge,0,0.13);radius*=THREE.MathUtils.lerp(.62,1,t)}
    r.push(radius);
  }
  return{path,radii:smoothRadii(r,2)};
}
function rebuild(x){
  x.mesh.geometry.dispose();
  const {path,radii}=resample(x.samples,x.settings);
  x.mesh.geometry=buildBalloon(path,radii,{sides:x.settings.sides,capRings:5});
  x.mesh.material.wireframe=$('#wire').checked;
}
function addItem(samples,settings=uiSettings(),autoSelect=true){const mesh=new THREE.Mesh(new THREE.BufferGeometry(),material()),x={samples,settings:{...settings},mesh};items.push(x);scene.add(mesh);rebuild(x);if(autoSelect)select(x);return x}
...
function loadControls(x){$('#width').value=x.settings.width;$('#pressure').value=x.settings.pressure;$('#smooth').value=x.settings.smooth;$('#sides').value=x.settings.sides;$('#taper').checked=x.settings.taper;syncOutputs()}
...
function setMode(m){mode=m;controls.enabled=m==='orbit';$('#drawBtn').classList.toggle('active',m==='draw');$('#orbitBtn').classList.toggle('active',m==='orbit');status.textContent=m==='orbit'?'Orbit: one finger rotate • two fingers zoom/pan':'Ready — draw a balloon stroke'}
function syncOutputs(){$('#widthOut').value=(+$('#width').value).toFixed(2);$('#pressureOut').value=`${Math.round(+$('#pressure').value*100)}%`;$('#smoothOut').value=$('#smooth').value;$('#sidesOut').value=$('#sides').value}
...
function serializeOBJ(){let out='# MeshUtilz Balloon v0.6.1\n',offset=1;for(let i=0;i<items.length;i++){const x=items[i],g=x.mesh.geometry,pos=g.getAttribute('position');if(!pos||pos.count<3)continue;const index=g.index;out+=`o Balloon_${i+1}\n`;x.mesh.updateMatrixWorld(true);for(let n=0;n<pos.count;n++){const v=new THREE.Vector3().fromBufferAttribute(pos,n).applyMatrix4(x.mesh.matrixWorld);out+=`v ${v.x} ${v.y} ${v.z}\n`}if(index){for(let n=0;n+2<index.count;n+=3)out+=`f ${index.getX(n)+offset} ${index.getX(n+1)+offset} ${index.getX(n+2)+offset}\n`}offset+=pos.count}return out}
function refreshExport(){const a=$('#exportBtn');if(exportUrl){URL.revokeObjectURL(exportUrl);exportUrl=null}const valid=items.some(x=>(x.mesh.geometry.getAttribute('position')?.count||0)>=3);if(!valid){a.classList.add('disabled');a.removeAttribute('download');a.href='#';return}exportUrl=URL.createObjectURL(new Blob([serializeOBJ()],{type:'text/plain;charset=utf-8'}));a.href=exportUrl;a.download='MeshUtilz-Balloon-v0.6.1.obj';a.classList.remove('disabled')}

renderer.domElement.addEventListener('pointerdown',e=>{
  if(mode==='orbit'){
    orbitTap={id:e.pointerId,x:e.clientX,y:e.clientY,moved:false};
    return;
  }
  const p=pointFromEvent(e);if(!p)return;checkpoint();drawing=true;current=addItem([{p,pressure:eventPressure(e)}],uiSettings(),false);renderer.domElement.setPointerCapture(e.pointerId);status.textContent='Drawing balloon…';
});
renderer.domElement.addEventListener('pointermove',e=>{
  if(mode==='orbit'){
    if(orbitTap&&orbitTap.id===e.pointerId&&Math.hypot(e.clientX-orbitTap.x,e.clientY-orbitTap.y)>6)orbitTap.moved=true;
    return;
  }
  if(!drawing||!current)return;const p=pointFromEvent(e);if(!p)return;const last=current.samples.at(-1).p;
  if(p.distanceTo(last)>.035){current.samples.push({p,pressure:eventPressure(e)});rebuild(current);updateStatus()}
});
...
renderer.domElement.addEventListener('pointerup',e=>{
  if(mode==='orbit'){
    if(orbitTap&&orbitTap.id===e.pointerId&&!orbitTap.moved){eventRay(e);const hits=ray.intersectObjects(items.map(x=>x.mesh),false);select(hits.length?items.find(x=>x.mesh===hits[0].object):null)}
    orbitTap=null;
    return;
  }
  finishStroke();
});
renderer.domElement.addEventListener('pointercancel',e=>{if(mode==='orbit'){orbitTap=null;return}finishStroke()});

$('#drawBtn').onclick=()=>setMode('draw');$('#orbitBtn').onclick=()=>setMode('orbit');
['width','pressure','smooth','sides','taper'].forEach(id=>$('#'+id).addEventListener('input',()=>{syncOutputs();if(selected)applySelected(false)}));
...
resize();syncOutputs();updateSelectionUI();updateStatus();refreshExport();renderer.setAnimationLoop(()=>{controls.update();renderer.render(scene,camera)});
