...
const controls=new OrbitControls(camera,renderer.domElement);controls.target.copy(WORLD_ORIGIN);controls.enableDamping=true;controls.enabled=true;controls.enableRotate=false;controls.enablePan=true;controls.enableZoom=true;controls.touches.TWO=THREE.TOUCH.DOLLY_PAN;
...
function setMode(m){mode=m;$('#drawBtn').classList.toggle('active',m==='draw');$('#orbitBtn').classList.toggle('active',m==='orbit');status.textContent=m==='draw'?'Draw: Pencil draws • finger orbits last balloon':'Orbit / Select: finger or Pencil rotates • tap selects'}
...
renderer.domElement.addEventListener('pointerdown',e=>{
  ...
  if(finger){e.stopImmediatePropagation();trackOrbitDown(e);return}
  if(mode==='orbit'){trackOrbitDown(e);return}
  e.stopImmediatePropagation();startDraw(e);
},{capture:true});
renderer.domElement.addEventListener('pointermove',e=>{
  const finger=e.pointerType==='touch';
  if(finger){e.stopImmediatePropagation();trackOrbitMove(e);return}
  if(mode==='orbit'){trackOrbitMove(e);return}
  e.stopImmediatePropagation();moveDraw(e);
},{capture:true});
renderer.domElement.addEventListener('pointerup',e=>{
  const finger=e.pointerType==='touch';
  if(finger){e.stopImmediatePropagation();finishOrbitTap(e);return}
  if(mode==='orbit'){finishOrbitTap(e);return}
  e.stopImmediatePropagation();finishStroke();
},{capture:true});
renderer.domElement.addEventListener('pointercancel',e=>{if(e.pointerType==='touch'||mode==='orbit'){orbitTap=null;return}e.stopImmediatePropagation();finishStroke()},{capture:true});
